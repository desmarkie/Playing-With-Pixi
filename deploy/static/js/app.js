...
BasicGrid = (function(_super) {
  __extends(BasicGrid, _super);

  function BasicGrid(renderer, name) {
    this.renderer = renderer;
    this.name = name;
    ...
    BasicGrid.__super__.constructor.call(this, this.renderer, this.name);
  }

  BasicGrid.prototype.load = function() {
    if (!this.loaded) {
      this.makeGui();
      this.sprites = [];
      this.pool = [];
      ...
      this.gridWidth = 25;
      this.gridHeight = 25;
      this.viewSize = Math.min(window.innerWidth, window.innerHeight) * 0.75;
      this.gridSize = this.viewSize / Math.max(this.gridWidth, this.gridHeight);
      this.gridHolder = new PIXI.DisplayObjectContainer();
      this.view.addChild(this.gridHolder);
      this.drawGrid();
      this.xpos = -1;
      this.ypos = -1;
      this.gui.add(this, 'gridWidth', 1, 200, 1);
      this.gui.add(this, 'gridHeight', 1, 200, 1);
      this.gui.add(this, 'drawGrid');
      this.gui.add(this, 'xpos').listen();
      this.gui.add(this, 'ypos').listen();
    }
    BasicGrid.__super__.load.call(this);
    return null;
  };

  BasicGrid.prototype.unload = function() {
    BasicGrid.__super__.unload.call(this);
    return null;
  };

  BasicGrid.prototype.update = function() {
    var ref;
    BasicGrid.__super__.update.call(this);
    if (this.cancelled) {
      return;
    }
    this.xpos = Math.round((window.app.pointerPosition.x - this.gridHolder.position.x) / this.gridSize) - 1;
    this.ypos = Math.round((window.app.pointerPosition.y - this.gridHolder.position.y) / this.gridSize) - 1;
    ref = this.xpos + '_' + this.ypos;
    if (this.xpos >= 0 && this.xpos < this.gridWidth && this.ypos >= 0 && this.ypos < this.gridHeight && this.grid.tiles[ref].sprite !== this.curTile) {
      if (this.curTile !== null) {
        TweenMax.to(this.curTile, 5, {
          alpha: 0,
          ease: Power4.easeOut
        });
        TweenMax.to(this.curTile.scale, 5, {
          x: 0,
          y: 0,
          ease: Power4.easeOut
        });
      }
      this.curTile = this.grid.tiles[ref].sprite;
      TweenMax.to(this.curTile, 0.3, {
        alpha: 1,
        ease: Power4.easeOut
      });
      TweenMax.to(this.curTile.scale, 0.3, {
        x: 1,
        y: 1,
        ease: Power4.easeOut
      });
  ...
  BasicGrid.prototype.drawGrid = function() {
    var cell, ref, t, _ref;
    this.grid = new Grid(this.gridWidth, this.gridHeight);
    this.gridSize = this.viewSize / Math.max(this.gridWidth, this.gridHeight);
    this.tileSprite = this.createTileSprite();
    while (this.sprites.length > 0) {
      this.pool.push(this.sprites.splice(0, 1)[0]);
    }
    _ref = this.grid.tiles;
    for (ref in _ref) {
      cell = _ref[ref];
      t = this.getTile(cell.x * this.gridSize, cell.y * this.gridSize);
      t.alpha = t.scale.x = t.scale.y = 0;
      cell.sprite = t;
      this.sprites.push(t);
      this.gridHolder.addChild(t);
    }
    this.gridHolder.position.x = (window.innerWidth - (this.gridWidth * this.gridSize)) * 0.5;
    this.gridHolder.position.y = (window.innerHeight - (this.gridHeight * this.gridSize)) * 0.5;
    return null;
  };

  BasicGrid.prototype.getTile = function(x, y) {
    var tile;
    tile = null;
    if (this.pool.length > 0) {
      tile = this.pool.splice(0, 1)[0];
      tile.setTexture(this.tileSprite);
    } else {
      tile = new PIXI.Sprite(this.tileSprite);
    }
    tile.anchor.x = tile.anchor.y = 0.5;
    tile.position.x = x + this.gridSize;
    tile.position.y = y + this.gridSize;
    return tile;
  };

  BasicGrid.prototype.createTileSprite = function() {
    var g;
    g = new PIXI.Graphics();
    g.beginFill(0xffffff);
    g.drawRect(0, 0, this.gridSize, this.gridSize);
    g.endFill();
